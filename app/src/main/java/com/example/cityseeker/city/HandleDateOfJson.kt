...
object HandleDateOfJson {
    ...
    private fun parseCities(context: Context , fileName: String): List<CityData> {
    ...
    fun getCities(context: Context , fileName: String): List<CityData> {
    ...
    fun findCityByName(context: Context , fileName: String , cityName: String): CityData? {
        val cities = parseCities(context , fileName)
        val index = cities.binarySearch { it.name.compareTo(cityName) }
        return if (index >= 0) cities[index] else null
    }
}
